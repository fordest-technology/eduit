...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (classData.schoolId !== subject.schoolId) {
      return NextResponse.json({ error: "Class and subject must belong to the same school" }, { status: 400 })
    }
    ...
    if (existingAssignment) {
      return NextResponse.json({ error: "Subject is already assigned to this class" }, { status: 400 })
    }
