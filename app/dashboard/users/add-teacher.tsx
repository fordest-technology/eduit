...
export function AddTeacherDialog({ open, onOpenChange, onSuccess, subjects, schoolId }: AddTeacherDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // If subjects are selected, assign them to the teacher
      if (formData.subjectIds.length > 0) {
        for (const subjectId of formData.subjectIds) {
          await fetch("/api/subject-teachers", {
            method: "POST",
            headers: {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Enter teacher details to create a new account.</DialogDescription>
        </DialogHeader>
